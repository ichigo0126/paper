...
export const getUserReviewsByReviewId = async (reviewId: string) => {
  // 特定のドキュメントへの参照を取得します。
  ...
  // ドキュメントが存在するかどうかを確認し、存在する場合はデータを返します。
  if (reviewDocSnapshot.exists()) {
    return reviewDocSnapshot.data();
  } else {
 ...
 */
export const createLikeForReview = async (userId: string, reviewId: string) => {
  // 'likes'コレクションへの参照を取得します。
  const likesRef = collection(db, "likes");
 ...
 * @param reviewId - いいねが付けられたレビューのID
 */
export const deleteLikeForReview = async (userId: string, reviewId: string) => {
  // 'likes'コレクションへの参照を取得します。
  const likesRef = collection(db, "likes");
...
export const getBookmarkByUserAndBook = async (
  ...
  const bookmarkDocRef = doc(db, "userBookmarks", userId, "bookmarks", bookId);
...
export const getUserFollowers = async (userId: string) => {
  ...
  return followerResults.filter(
    (follower): follower is Exclude<typeof follower, null> => follower !== null
  );
};
...
async function signInWithGoogle() {
  try {
    ...
    if (user.displayName && user.email) {
      ...
      console.log(
        "User signed in with Google and user document checked/initialized"
      );
    } else {
